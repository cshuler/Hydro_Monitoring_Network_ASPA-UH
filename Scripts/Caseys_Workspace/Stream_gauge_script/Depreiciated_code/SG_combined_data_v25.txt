...
def normalize_station_name(station_name, base_directory):
    ...
    for folder in os.listdir(base_directory):
        ...
        # Split the cleaned station name and folder name into number and name parts
        folder_number_part = folder_clean.split()[0]
        folder_name_part = " ".join(folder_clean.split()[1:])

        # Check if the number part and the name part match
        if station_number in folder_number_part and station_name_clean.split(station_number)[-1] in folder_name_part:
