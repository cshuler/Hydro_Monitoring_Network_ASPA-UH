...
def process_station_files(input_directory, output_file, station_name, start_date, end_date):
    ...
    # Traverse through directories but only include the "Data" folder and exclude "Master data sheet"
    for root, dirs, files in os.walk(station_folder):
        dirs[:] = [d for d in dirs if d == 'Data' and 'Master data sheet' not in root]  # Limit directories
...
def main():
    ...
    station_name = input("Enter the stream gauge name (e.g., 1410_Fagalii): ")  # Choose specific station folder
