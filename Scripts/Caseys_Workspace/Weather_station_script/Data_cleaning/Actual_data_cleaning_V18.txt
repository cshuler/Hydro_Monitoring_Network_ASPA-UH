...
# Define conditions for identifying "bad data" in columns, with type checks and issue identification
def create_bad_data_conditions():
    conditions = {
        'SWin_Avg': lambda x: 'Negative number' if isinstance(x, (int, float)) and x < 0 else 'NAN' if pd.isna(x) else None,
        'Tair_Avg': lambda x: 'Negative number' if isinstance(x, (int, float)) and x < 0 else 'NAN' if pd.isna(x) else None,
        ...
        'WS_Avg': lambda x: 'Negative number' if isinstance(x, (int, float)) and x < 0 else 'NAN' if pd.isna(x) else None,
        'WSrs_Avg': lambda x: 'Negative number' if isinstance(x, (int, float)) and x < 0 else 'NAN' if pd.isna(x) else None,
        'WDuv_Avg': lambda x: 'Out of range' if isinstance(x, (int, float)) and (x < 0 or x > 360) else 'NAN' if pd.isna(x) else None,
        'WD_StdY': lambda x: 'Out of range' if isinstance(x, (int, float)) and (x < 0 or x > 360) else 'NAN' if pd.isna(x) else None,
        'RF_Tot': lambda x: None,  # No specific condition
    }
    return conditions
            ...
            if col in pt_data.columns:
                ...
                if issue:  # If an issue is identified
                    bad_data_records.append({
                        'Bad data Start': date_time.strftime('%m/%d/%Y %H:%M'),  # Format date and time
                        'Bad data End': date_time.strftime('%m/%d/%Y %H:%M'),
                        'Data affected': col,
                        'No data from': '',
                        'To': '',
                        'Notes': issue
