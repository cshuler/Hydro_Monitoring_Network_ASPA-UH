...
def process_and_filter_bad_data(input_directory, start_date, end_date, output_file):
    ...
    if combined_pt_data:
        pt_data_combined = pd.concat(combined_pt_data, ignore_index=True)
    else:
        pt_data_combined = pd.DataFrame()

    if all_bad_data_records:
        bad_data_combined = pd.concat(all_bad_data_records, ignore_index=True)
    else:
        bad_data_combined = pd.DataFrame(columns=['Bad data Start', 'Bad data End', 'Data affected', 'Notes'])
