...
def create_bad_data_conditions():
    conditions = {
        'SWin_Avg': lambda x: x < 0 or pd.isna(x),
        'Tair_Avg': lambda x: x < 0 or pd.isna(x),
        'RH_Avg': lambda x: x < 10 or x > 100 or pd.isna(x),
        'WS_Avg': lambda x: x < 0 or pd.isna(x),
        'WSrs_Avg': lambda x: x < 0 or pd.isna(x),
        'WDuv_Avg': lambda x: x < 0 or x > 360 or pd.isna(x),
        'WD_StdY': lambda x: x < 0 or x > 360 or pd.isna(x),
...
def process_and_filter_bad_data(input_directory, start_date, end_date, station_name, output_file_base):
    ...
    for root, dirs, files in os.walk(input_directory):
        for file_name in files:
            ...
            date_match = re.search(rf"{start_date.strftime('%-m.%-d.%Y')}", file_name) or re.search(rf"{end_date.strftime('%-m.%-d.%Y')}", file_name)
            if date_match:
                matched_files += 1
