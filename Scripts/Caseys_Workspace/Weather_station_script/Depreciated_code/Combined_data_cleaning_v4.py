...
def clean_weather_data(file_path):
    try:
        ...
        if 'PT data' in xl.sheet_names:
            ...
            df = pd.read_excel(file_path, sheet_name='MetData')  # Fallback to 'MetData' sheet if needed
        else:
...
def process_multiple_files_to_single_sheet(input_dir, output_file, start_date, end_date):
    ...
    if combined_df.empty:
        ...
        try:
            # Ensure the file is saved as an .xlsx file with openpyxl engine
...
def main():
    ...
    # Set input and output directories based on the station name
    ...
    # Process files and combine into a single output
