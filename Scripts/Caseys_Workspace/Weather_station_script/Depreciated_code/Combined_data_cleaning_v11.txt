...
def process_multiple_files_to_single_sheet(input_directory, output_file, start_date, end_date):
    all_data = []
    print("Starting the script...")  # Adding a print statement to indicate script start
    ...
    if all_data:
        ...
        print(f"Saving data to {output_file}")  # Print statement before saving the file
        try:
            combined_df.to_excel(output_file, index=False, engine='openpyxl')
            logging.info(f"Data successfully saved to {output_file}")
            print(f"Data saved to: {output_file}")  # Print statement after saving the file
        except Exception as e:
            print(f"An error occurred while saving the file: {e}")

    print("Script completed.")  # Print statement to indicate the script has finished
