...
def clean_weather_data(file_path):
    try:
        ...
        if 'PT data' in xl.sheet_names:
            df = pd.read_excel(file_path, sheet_name='PT data')  # Removed skiprows=3
        elif 'PT data (5min)' in xl.sheet_names:
            df = pd.read_excel(file_path, sheet_name='PT data (5min)')  # Removed skiprows=3
        else:
        ...
        # Reindex the DataFrame to match the desired columns and leave missing ones blank
        df_cleaned = df.reindex(columns=desired_columns, fill_value=None)
