...
def clean_weather_data(file_path):
    try:
        ...
        # Check if 'PT data' or 'PT data (5min)' sheets exist, if not load the first sheet instead
        xls = pd.ExcelFile(file_path)
        sheet_name_to_use = None
        if 'PT data' in xls.sheet_names:
            sheet_name_to_use = 'PT data'
        elif 'PT data (5min)' in xls.sheet_names:
            sheet_name_to_use = 'PT data (5min)'
        else:
            logging.warning(f"'PT data' and 'PT data (5min)' sheets not found in {file_path}. Trying the first available sheet.")
            sheet_name_to_use = xls.sheet_names[0]  # Fallback to first sheet

        # Read the identified sheet
        df = pd.read_excel(file_path, sheet_name=sheet_name_to_use, skiprows=3)
        ...
        df_cleaned = df_cleaned.reindex(columns=variable_mapping.keys(), fill_value='')
...
def extract_date_from_filename(filename):
    ...
    # Define a regular expression to capture date-like patterns
